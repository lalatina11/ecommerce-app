    ...
    return (
        <div className={"w-full"}>
            ...
            <div className="flex flex-col gap-2 w-full">
                {list.map((item) => (
                    <Card key={item.id} className={"flex-row items-center justify-between 2xl:gap-4 p-3"}>
                        <div className={"w-12 h-12 min-w-12 rounded-sm overflow-hidden relative"}>
                            <Image src={item.image} alt={item.title} fill
                                   className={"object-cover min-w-12 min-h-auto"}/>
                        </div>
                        <CardContent className={"p-0 flex-1"}>
                            <CardTitle className={"2xl:text-xs"}>{item.title}</CardTitle>
                            <Badge variant={"secondary"}>{item.badge}</Badge>
                        </CardContent>
                        <CardFooter className={"2xl:text-xs p-0"}>{item.count / 1000}K</CardFooter>
                    </Card>
                ))}
            </div>
        </div>
